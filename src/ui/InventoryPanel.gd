...
func _build_rule_slots() -> void:
    ...
    for i in GameState.rule_slots.size():
        ...
        t_btn.get_node("MarginContainer/HBox/Label").text = (t_comp.display_name + " [%d/%.0f]" % [t_comp.trigger_count, t_comp.trigger_value]) if t_comp else "[T 空]"
        if t_comp != null:
            var t_tex := ComponentIcons.get_icon(t_comp.id)
            if t_tex != null:
                t_btn.get_node("MarginContainer/HBox/Icon").texture = t_tex
        ...
        e_btn.get_node("MarginContainer/HBox/Label").text = (e_comp.display_name + " [%.1f]" % e_comp.effect_value) if e_comp else "[E 空]"
        if e_comp != null:
            var e_tex := ComponentIcons.get_icon(e_comp.id)
            if e_tex != null:
                e_btn.get_node("MarginContainer/HBox/Icon").texture = e_tex
...
func _build_inventory_grid() -> void:
    ...
    for comp in GameState.inventory:
        ...
        btn.get_node("MarginContainer/HBox/Label").text = label
        var icon_tex := ComponentIcons.get_icon(comp.id)
        if icon_tex != null:
            btn.get_node("MarginContainer/HBox/Icon").texture = icon_tex
        var c = comp
