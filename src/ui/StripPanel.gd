...
func _make_card(comp: ComponentData) -> PanelContainer:
    ...
    card.get_node("VBox/InfoRow/Label").mouse_entered.connect(func(): Tooltip.show_tip(comp.description))
    card.get_node("VBox/InfoRow/Label").mouse_exited.connect(Tooltip.hide_tip)
